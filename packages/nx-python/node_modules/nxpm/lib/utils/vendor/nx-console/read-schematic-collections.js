...
function readWorkspaceJsonDefaults(workspaceJsonPath) {
    const defaults = utils_1.readAndParseJson(workspaceJsonPath).schematics || {};
...
async function readSchematicCollectionsFromNodeModules(workspaceJsonPath) {
    ...
    const schematicCollections = packages.filter((p) => {
        try {
            return !!utils_1.readAndCacheJsonFile(path_1.join(p, 'package.json'), nodeModulesDir).json.schematics;
        }
        catch (e) {
            if (e.message &&
                (e.message.indexOf('no such file') > -1 || e.message.indexOf('not a directory') > -1)) {
                return false;
            }
            else {
                throw e;
...
async function readCollection(basedir, collectionName, defaults) {
    try {
        ...
        const collection = utils_1.readAndCacheJsonFile(packageJson.json.schematics, path_1.dirname(packageJson.path));
...
async function readAllSchematicCollections(workspaceJsonPath, workspaceSchematicsPath = path_1.join('tools', 'schematics')) {
    const basedir = path_1.join(workspaceJsonPath, '..');
...
async function readCollectionSchematics(collectionName, collectionPath, collectionJson, defaults) {
    ...
    Object.entries(collectionJson.schematics).forEach(async ([k, v]) => {
        try {
            if (canAdd(k, v)) {
